...
const METABOLIC_INDEX = null;
function _mi() { return window.metaboliteData || []; }
        ...
        for (const pattern of patterns) {
            if (text.toLowerCase().includes(pattern.toLowerCase())) {
                ...
                if (match && parseFloat(match[1]) > 0) {
                    ...
                    const fieldDef = _mi().find(f => f.id === field);
...
function fallbackOCR(imageSrc) {
    ...
    const interval = setInterval(() => {
        ...
        if (progress >= 95) {
            ...
            const randomizedFields = shuffleArray(_mi()).slice(0, Math.floor(Math.random() * 6) + 4);
...
function loadMetabolicHistory(filter = 'all') {
    ...
    thead.innerHTML = `<tr style="background: #f8fafc; border-bottom: 2px solid #e2e8f0;">
        ...
        ${(window.metaboliteData||[]).map(f => `<th style="padding: 14px 12px; text-align: center; font-weight: 800; color: #475569;">${f.name}</th>`).join('')}
    ...
    if (records.length === 0) {
        const colSpan = (window.metaboliteData||[]).length + 3;
    ...
    tbody.innerHTML = records.map((record, index) => {
        ...
        const cellsHtml = (window.metaboliteData||[]).map(f => `<td style="padding: 12px; text-align: center;">${cell(f)}</td>`).join('');
